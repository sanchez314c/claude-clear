...
# Add upstream remote
git remote add upstream https://github.com/sanchez314c/claude-clear.git
git fetch upstream
```
...
### Language
- **Python 3.8+** — cross-platform, rich stdlib, no compilation required
- All core functionality uses Python stdlib only (`json`, `pathlib`, `datetime`, `argparse`, `subprocess`, `shutil`, `sys`, `os`)

### Runtime Dependencies (`requirements.txt`)
| Package | Version | Usage |
|---------|---------|-------|
| `click` | >=8.0.0 | Listed for future CLI refactor (argparse used currently) |
| `PyYAML` | >=6.0 | Listed for future config file support (not imported currently) |
...
- **Type hints**: All functions should use typing annotations
...
# Output: dist/claude-clear-1.0.0-py3-none-any.whl
...
make clean         # remove build/, dist/, *.egg-info/, __pycache__
...
- **feature/\*** — new features (`feature/config-validation`)
...
4. Open PR against `develop` with PR template filled out
...
- **GitHub Issues**: [https://github.com/sanchez314c/claude-clear/issues](https://github.com/sanchez314c/claude-clear/issues)