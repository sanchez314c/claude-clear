...
**What is Claude Clear?** — A Python utility that cleans `~/.claude.json` by removing accumulated chat history and cache data while preserving settings, API keys, and MCP configs.

**How much space does it save?** — Typically reduces a 27MB config to under 100KB (~99.6% reduction).

**Is it safe?** — Yes. It creates a timestamped backup before any changes. You can always restore.

**What gets removed?** — `history`, `conversation`, `messages`, `chat`, `conversations`, `messageHistory`, `chatHistory`, `contextCache` per project; plus `globalHistory`, `globalMessages`, `conversations`, `recentConversations`, `conversationCache`, `chatCache` at the top level.

**What is preserved?** — User settings, API keys, MCP configurations, project metadata, tool configurations.

**How often should I run it?** — Light users: monthly. Regular users: weekly. Heavy users: daily or every few days.

**Does it send data anywhere?** — No. All processing is local. Nothing is transmitted or logged externally.

**Can I automate it?** — Yes. Use cron (Linux/macOS), LaunchAgent (macOS), or Task Scheduler (Windows). See [INSTALLATION.md](INSTALLATION.md).
...
2. **Search Issues**: Look at [GitHub Issues](https://github.com/sanchez314c/claude-clear/issues)
